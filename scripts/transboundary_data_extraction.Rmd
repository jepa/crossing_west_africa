...
wa_eez <- c("Senegal","Mauritania","Western Sahara","Morocco (South)","Morocco (Central)","Canary Isl. (Spain)","Madeira Isl. (Portugal)","Cape Verde","Gambia")

# Load SAU sf
sau_eez <- my_sf("SAU")

sau_eez %>% 
  filter(name %in% wa_eez) %>% 
  ggplot() +
  geom_sf(
    aes(
      color = name,
...
ssr_to_read <- paste0(ssr_data_path,"proportion_",unique(wa_trans_spp$taxon_key),".csv")


nwa_ssr_data <- bind_rows(
    lapply(ssr_to_read, read_proportion, eez = wa_eez)
  )
